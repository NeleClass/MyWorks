...
public class PersonController {
    ...
    @RequestMapping(method = RequestMethod.GET, path = "ws/{id}")
    @ResponseBody
    public Person getOneWS(@PathVariable Integer id) {
        return personDAO.getOne(id);
    }

    @RequestMapping(path = "ws/{id}", method = {RequestMethod.PUT, RequestMethod.POST})
    @ResponseBody
    @Transactional
    public Person saveWs(
            @PathVariable(name = "id", required = false) String id,
            @RequestBody Person paramPerson
    ) {
        Person person = null;
        try {
        ...
        person.setFirstName(paramPerson.getFirstName());
        person.setLastName(paramPerson.getLastName());
        person.setBirthDate(paramPerson.getBirthDate());
        person.setSalary(paramPerson.getSalary());
        // Saving object to DB
    ...
    @RequestMapping(path = "ws/new", method = RequestMethod.POST)
    @ResponseBody
    @Transactional
    public Person saveWsNew(
            @RequestBody Person paramPerson
    ) {
        Person person = null;
        if (person == null) {
